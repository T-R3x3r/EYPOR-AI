...
PLOTLY SUBPLOT COMPATIBILITY RULES (CRITICAL):
- NEVER mix table traces (go.Table) with XY subplots (go.Bar, go.Scatter, go.Heatmap) in the same subplot grid
- If you need both charts and tables, create separate figures or use different subplot types
- For subplots with mixed content:
  - Use specs parameter in make_subplots to specify subplot types
  - For tables: specs=[[{{"type": "table"}}], [{{"type": "xy"}}]]
  - For charts only: specs=[[{{"type": "xy"}}], [{{"type": "xy"}}]]
- If creating a single figure with multiple chart types, use separate figures or ensure all traces are compatible
- Table traces can only be added to subplots with type="table"
- XY traces (bar, scatter, line) can only be added to subplots with type="xy"
- When using make_subplots, always specify the correct subplot types in the specs parameter

COMMON PLOTLY PITFALLS TO AVOID:
- Never use fig.show() - only save as HTML file
- Always convert pandas Series/DataFrame to Python lists using .tolist() before plotting
- Reset DataFrame indexes using df.reset_index(drop=True) before extracting data
- Ensure all data arrays contain actual values, not DataFrame indexes
- For horizontal bar charts: use orientation='h' and yaxis autorange='reversed'
- For subplots: always specify subplot types in make_subplots specs parameter
- For tables: use go.Table, not go.Bar with table-like data
- For heatmaps: use go.Heatmap, not go.Bar with 2D data
- Always use absolute database paths, not relative paths
- Save HTML files in the current directory with dynamic timestamps

SQL QUERY REQUIREMENTS:
- NEVER use reserved keywords as column aliases (e.g., 'Table', 'Order', 'Group', 'Index', 'Key')
- Use descriptive aliases like 'TableName', 'TableSource', 'SourceTable' instead of 'Table'
- Always quote table and column names that might be reserved keywords
- Use proper SQL syntax for SQLite (no backticks, use double quotes for identifiers if needed)
- Test queries with simple SELECT statements first before complex joins
- Handle missing tables gracefully with try/except blocks
- Use parameterized queries to prevent SQL injection

SUBPLOT SPECIFICATIONS FOR MIXED CONTENT:
- When combining charts and tables in subplots, ALWAYS use specs parameter:
  - For charts + tables: specs=[[{{"type": "xy"}}], [{{"type": "table"}}]]
  - For charts only: specs=[[{{"type": "xy"}}], [{{"type": "xy"}}]]
  - For tables only: specs=[[{{"type": "table"}}], [{{"type": "table"}}]]
- Example: make_subplots(rows=2, cols=1, specs=[[{{"type": "xy"}}], [{{"type": "table"}}]])
- NEVER add table traces to XY subplots or vice versa
- If unsure, create separate figures for charts and tables
